...
*/
#include "LCD_Display.h" /* Arquivo de cabeçalho para referenciar os prótipos de funções e as definifições solicidas*/

unsigned char bits_pinos[4] = {LCD_BIT_4, LCD_BIT_5, LCD_BIT_6, LCD_BIT_7 };/*Vetor contendo os pinos de dados */
...
void LCD_Escreve_Nibble( unsigned char dado){
  ...
  */
  ...
  for (int i = 0; i < 4; i++) {
    digitalWrite(bits_pinos[i], (dado >> i) & 0x01);
  }
  LCD_Habilita();
...
void LCD_Iniciar (void){
   ...
   pinMode(LCD_BIT_4, OUTPUT); 
   pinMode(LCD_BIT_5, OUTPUT);
   pinMode(LCD_BIT_6, OUTPUT); 
   pinMode(LCD_BIT_7, OUTPUT);
